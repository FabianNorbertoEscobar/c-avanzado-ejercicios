...
void diferencia_simetrica_de_archivos_binarios(FILE *pf_A,FILE *pf_B,FILE *pf_dif_sim)
    {
    int comparador;
    ...
    while(!feof(pf_A)&&!feof(pf_B))
        ...
        if(comparador<0)
            {
            fwrite(&persona_A,sizeof(t_persona),1,pf_dif_sim);
            fread(&persona_A,sizeof(t_persona),1,pf_A);
            }
        else
            if(comparador>0)
                {
                fwrite(&persona_B,sizeof(t_persona),1,pf_dif_sim);
                fread(&persona_B,sizeof(t_persona),1,pf_B);
                }
            else
                {
